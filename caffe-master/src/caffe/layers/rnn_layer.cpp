...
void RNNLayer<Dtype>::FillUnrolledNet(NetParameter* net_param) const {
  ...
  const FillerParameter& rnn_recweight_filler = 
      this->layer_param_.recurrent_param().rnn_recweight_filler();
  ...
  LayerParameter rec_hidden_param(hidden_param);
  rec_hidden_param.mutable_inner_product_param()->
      mutable_weight_filler()->CopyFrom(rnn_recweight_filler);
  ...
  LayerParameter actvFunc_param;
  actvFunc_param.set_type(this->layer_param_.recurrent_param().rnn_actvfunc());
  ...
  input_layer_param->mutable_input_param()->add_shape()->CopyFrom(input_shapes[0]);
  ...
  for (int t = 1; t <= this->T_; ++t) {
    ...
    //     W_hh_h_{t-1} := W_hh * h_conted_{t-1}
    {
      ...
      w_param->CopyFrom(rec_hidden_param);
    ...
    //          = \tanh( W_hh_h_{t-1} + W_xh_t )
    {
      ...
      if (this->static_input_ && (t==1 || !this->layer_param_.recurrent_param().static_input_only_start())) {
    ...
    }
    {
      ...
      h_neuron_param->CopyFrom(actvFunc_param);
    ...
    //          = \tanh( W_ho_h_t )
    {
      ...
      o_neuron_param->CopyFrom(actvFunc_param);
