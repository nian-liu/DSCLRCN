...
void LSTMLayer<Dtype>::FillUnrolledNet(NetParameter* net_param) const {
  ...
  input_layer_param->mutable_input_param()->add_shape()->CopyFrom(input_shapes[1]);
  ...
  for (int t = 1; t <= this->T_; ++t) {
    ...
    //         h_t := o_t .* \tanh[c_t]
    {
	  ...
	  lstm_unit_param->mutable_lstm_unit_param()->set_additional_forget_bias(this->layer_param_.recurrent_param().lstm_additional_forget_bias());
    }
